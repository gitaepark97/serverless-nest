...
export class CreateUsers1696220692132 implements MigrationInterface {
  name = 'CreateUsers1696220692132';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`users\` (\`user_id\` int NOT NULL AUTO_INCREMENT, \`email\` varchar(50) NOT NULL, \`hashed_password\` varchar(255) NOT NULL, \`salt\` varchar(255) NOT NULL, \`nickname\` varchar(50) NOT NULL, \`gender\` enum ('0', '1') NOT NULL, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP, UNIQUE INDEX \`nickname\` (\`nickname\`), UNIQUE INDEX \`email\` (\`email\`), UNIQUE INDEX \`IDX_97672ac88f789774dd47f7c8be\` (\`email\`), UNIQUE INDEX \`IDX_ad02a1be8707004cb805a4b502\` (\`nickname\`), PRIMARY KEY (\`user_id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
